...
3. **양자 병렬 계산:** 중첩된 $x$값들에 대해 $f(x) = a^x \pmod N$을 계산하여 함숫값을 얻는다. 이 과정에서 제어 레지스터 $|0\rangle$의 위상에 함숫값에 대한 정보가 각인되는 위상 킥백(Phase Kickback) 현상이 발생한다. *(이는 나중에 위상 추정 파트에서 자세히 다룰 것이라 여기서는 그냥 그렇구나하고 넘어가자)*
